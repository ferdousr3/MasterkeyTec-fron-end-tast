...
const Partition: React.FC<PartitionProps> = ({ color, onRemove }) => {
  ...
  const splitVertical = () => {
    setIsVertical(true);
    setChildren([{ id: 1, color }, { id: 2, color: getRandomColor() }]);
  };
  ...
  return (
    <PartitionContainer color={color}>
      {children.length === 0 ? (
        <Buttons>
          <button onClick={splitVertical}>V</button>
          <button onClick={splitHorizontal}>H</button>
          {onRemove && <button onClick={onRemove}>-</button>}
        </Buttons>
      ) : (
        <SplitContainer isVertical={!!isVertical}>
          <Child size={size} isVertical={!!isVertical}>
            <Partition color={children[0].color} onRemove={() => setChildren([])} />
          </Child>
          <Resizer
          ...
          <Child size={100 - size} isVertical={!!isVertical}>
            <Partition color={children[1].color} onRemove={() => setChildren([])} />
          </Child>
        </SplitContainer>
      )}
    </PartitionContainer>
